...
### ES-C - project template migration
- goal:
  - 用新模板收紧 one-publish 的 roadmap/tasks
- validation:
  - 文档结构对齐模板
  - 不引入双重真相
- done_definition:
  - one-publish 成为第一份真实迁移样板
- risk:
  - medium

#### Slice C1 - align one-publish roadmap to contract-backed format
- phase_id: `PH-3`
- goal:
  - 为 one-publish roadmap 增加 contract reference，并去掉不该承载的运行态意味
- scope:
  - `contracts/one-publish-contract.md`
...
#### Slice C2a - align one-publish backend task schema
- phase_id: `PH-3`
- goal:
  - 为 one-publish tasks 中 Phase 2 的 backend slices 补齐 phase_id / rollback_strategy / 结构口径
- scope:
...
- done_definition:
  - one-publish 的 backend slices 与模板口径更一致
- rollback_strategy:
  - revert tasks edit commit
- risk:
  - medium

#### Slice C2b - align one-publish frontend hook task schema
- phase_id: `PH-3`
- goal:
  - 为 one-publish tasks 中 Phase 3 的 frontend hook slices 补齐 phase_id / rollback_strategy / 结构口径
- scope:
  - `tasks/one-publish-refactor-tasks.md`
- target_files:
  - `tasks/one-publish-refactor-tasks.md`
...
- done_definition:
  - one-publish 的 frontend hook slices 与模板口径更一致
- rollback_strategy:
  - revert tasks edit commit
- risk:
  - medium
...
- phase_id: `PH-3`
- goal:
  - 检查 one-publish 剩余 legacy slices 是否还存在明显 schema 缺口，并决定是否继续补齐
- scope:
  - `tasks/one-publish-refactor-tasks.md`
  - `roadmaps/one-publish-refactor-roadmap.md`
- target_files:
...
- validation:
  ...
  - `python3 /Users/erictao/source/repos/six-layer-execution-system/scripts/check_active_consistency.py` 通过
- done_definition:
...
- depends_on:
  - `ES-F.F5`
- validation:
  - 用 `tasks/one-publish-refactor-tasks.md` 标出哪些 slices 属于 in-scope migrated slices
  - 明确实现不应把 `Task 1/2/3...` 这类 legacy task headings 当成 migrated slices
...
- done_definition:
  - 第二条 checker 的第一版实现可运行并覆盖 one-publish 样本
- rollback_strategy:
...
- rollback_strategy:
  - revert smoke additions if they overfit one-publish wording instead of the checker contract boundary
- risk:
  - medium
...
- validation:
  ...
  - one-publish 样本上能识别出首批真正偏宽的 migrated slices
- done_definition:
...
- rollback_strategy:
  - revert smoke additions if they overfit one-publish wording instead of the advisory contract trigger
- risk:
  - low
...
- validation:
  ...
  - `python3 /Users/erictao/source/repos/six-layer-execution-system/scripts/run_execution_system_checks.py` 通过
- done_definition:
