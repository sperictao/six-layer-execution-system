...
## Current phase
- Phase 5 - Checker and workflow adoption (`DX-E / Slice E10 - hand implementation focus back to one-publish-refactor while preserving execution-system reopen state`)
...
- integration_notes:
  - 与 one-publish contract 现有约束语义保持兼容
- handoff_output:
  - guardrail_fields
  - compatibility_notes
- validation:
...
- validation:
  - `python3 /Users/erictao/source/repos/six-layer-execution-system/scripts/check_active_wave_state.py`
  - `python3 /Users/erictao/source/repos/six-layer-execution-system/scripts/test_check_active_wave_state.py`
  - `python3 /Users/erictao/source/repos/six-layer-execution-system/scripts/check_active_consistency.py`
  - `python3 /Users/erictao/source/repos/six-layer-execution-system/scripts/run_execution_system_checks.py`
...
- handoff_output:
  ...
  - second_test_cut_candidate (`maintenance-mode vs approved non-business focus drift`)
...
- done_definition:
  ...
  - 第二优先级候选已明确为 `maintenance-mode vs approved non-business focus drift`
...
#### Slice E10 - hand implementation focus back to one-publish-advisory-convergence
- phase_id: `PH-5`
- goal:
  - 在不丢失 execution-system 上下文的前提下，将下一条真正需要实现收敛的主线切回 `one-publish-advisory-convergence`
- scope:
  - `ACTIVE.md`
  - `roadmaps/execution-system-decomposition-upgrade-roadmap.md`
  - `tasks/execution-system-decomposition-upgrade-tasks.md`
...
- shared_write_targets:
  ...
  - `tasks/execution-system-decomposition-upgrade-tasks.md`
- expected_artifacts:
  - formal handoff state from decomposition-upgrade to one-publish-advisory-convergence
...
- integration_notes:
  - 这一步不关闭 execution-system activity 本身；只把“下一条实现主线”明确交回 one-publish 侧，并保留当前 activity 的可恢复痕迹
- handoff_output:
...
- done_definition:
  ...
  - 下一条实现主线明确为 `one-publish-advisory-convergence`
