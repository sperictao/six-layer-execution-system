...
- use `python3 /Users/erictao/source/repos/six-layer-execution-system/scripts/run_execution_system_checks.py` for routine checker validation
- use `python3 /Users/erictao/source/repos/six-layer-execution-system/scripts/check_active_consistency.py` when you only need the ACTIVE runtime truth check or are repairing `ACTIVE.md`
- use `python3 /Users/erictao/source/repos/six-layer-execution-system/scripts/check_task_slice_schema.py` when you are iterating specifically on migrated task-doc structure
...
4. rerunning `python3 /Users/erictao/source/repos/six-layer-execution-system/scripts/check_active_consistency.py`
...
#### Adoption gate
This checker should remain scoped to clearly migrated slices until at least one real project demonstrates that partial migration can coexist safely with legacy sections.

#### Current reference sample
`tasks/one-publish-refactor-tasks.md` is the current reference sample for this checker design because it contains both:
- clearly migrated slices with stable `phase_id` and `rollback_strategy` usage
- older task sections that should remain out of scope

The second checker should therefore behave as follows on that sample:
...
A practical deterministic boundary on the current sample is:
- headings like `#### Task 1`, `#### Task 2`, ... remain legacy and out of scope
- headings like `#### Slice 2A`, `#### Slice C1`, `#### Slice P2`, ... are candidates for in-scope migrated slices
...
- in `tasks/one-publish-refactor-tasks.md`, `#### Slice P2 - first frontend shell-collapse cut` is the kind of broad slice this advisory is meant to consider first
