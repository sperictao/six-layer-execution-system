...
- 任何 `tests/test_*.py` 单测命令都按 source checkout 根视角运行，并带 `PYTHONPATH="plugins/six-layer-execution-system:plugins/six-layer-execution-system/scripts"`
- `run_execution_system_full_tests.py` 只在 source checkout 根 `tests/` 存在时作为完整基线；standalone plugin 副本里 `unavailable` 是预期边界
...
- Phase 5 - Checker and workflow adoption (`DX-E.E10` is the handoff closeout: execution-system-decomposition-upgrade keeps its recovery path, while default review focus returns to `waiting-ledger-review`)
