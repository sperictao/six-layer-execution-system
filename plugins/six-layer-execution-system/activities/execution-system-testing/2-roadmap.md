...
- repo-local 开发测试位于 source checkout 根 `tests/`；直接执行单测时统一使用仓库根视角命令与显式 `PYTHONPATH`
- `run_execution_system_full_tests.py` 只在 source checkout 根 `tests/` 存在时提供完整 suite；standalone plugin 场景下 `unavailable` 是允许结果
- 现有 smoke 全绿
- 不破坏当前非 execution-system focus 的默认推进
