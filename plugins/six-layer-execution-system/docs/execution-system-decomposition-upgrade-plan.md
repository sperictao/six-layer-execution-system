...
### Objective
Extend contract files so they define decomposition guardrails, not only project-specific constraints.
...
5. `tests/test_execution_system_path_parallel_wave.py`
...
- [ ] `python3 scripts/check_active_wave_state.py` passes
- [ ] `PYTHONPATH="plugins/six-layer-execution-system:plugins/six-layer-execution-system/scripts" python3 tests/test_check_active_wave_state.py` passes
- [ ] `python3 scripts/check_active_consistency.py` passes with the pilot activity still active
...
The first pilot should use execution-system testing work, not external project runtime work.
...
- do not require git mutation outside the execution-system repo
- prefer direct checker/runner invocation over broad end-to-end environment setup

Explicit non-goals:
- do not execute external project build/test commands as part of this path test
- do not simulate real external-project runtime semantics
...
- no external-project runtime coupling is required for the first implementation wave
...
- a production-scale parallel-wave rollout across external project work
...
- pilot on execution-system testing work before external-project adoption
...
2. maintenance-mode vs approved non-execution-system focus drift
- target drift:
  - maintenance checks assuming only runnable work outside the execution-system line is valid, while the workspace may intentionally focus a guarded execution-system activity
- why it matters:
  - this already produced real test drift during the current wave
- suggested cut:
  - extend maintenance/governance docs and tests so approved non-execution-system focus states are explicitly modeled
- implementation anchors:
  - `scripts/check_execution_system_maintenance_mode.py`
  - `tests/test_execution_system_maintenance_mode.py`
  - `tests/test_execution_system_path_maintenance_focus_drift.py`

### Recommended second governance-drift test cut
`governance-drift-test-cut-2` should target maintenance-mode vs approved non-execution-system focus drift.
