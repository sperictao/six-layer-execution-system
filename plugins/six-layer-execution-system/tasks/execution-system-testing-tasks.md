...
- `run_execution_system_full_tests.py` 在 standalone plugin 副本里返回 `unavailable` 属于预期边界。
...
- depends_on:
  - `ET-C.C3`
- validation:
  - 从 execution-system maintenance focus 切回非 execution-system focus 时，runner / acceptance / smoke 都不假红
- done_definition:
...
- phase_id: `PH-4`
- goal:
  - 检查 execution-system parked / maintenance posture 与当前非 execution-system focus 是否一致
- scope:
  - `ACTIVE.md`
  - `scripts/`
- target_files:
...
- depends_on:
  - `ET-A.A1`
- validation:
  - execution-system parked + 非 execution-system focus 可运行 + acceptance green 能同时成立
- done_definition:
...
- target_files:
  ...
  - `tests/test_execution_system_path_*.py`
  - `docs/execution-system-testing-inventory.md`
  - `roadmaps/execution-system-testing-roadmap.md`
  - `tasks/execution-system-testing-tasks.md`
- depends_on:
...
- validation:
  ...
  - `python3 scripts/run_execution_system_full_tests.py` 通过
