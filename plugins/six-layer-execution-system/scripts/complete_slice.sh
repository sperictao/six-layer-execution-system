...
modes:
  ...
  payload  print the canonical notification-ready payload from the completed-slice artifact
...
case "$mode" in
  prepare)
    ...
    repo="$(focus_scalar repo 2>/dev/null || true)"
    if [[ -z "$repo" ]]; then
      repo_path="$(focus_scalar path 2>/dev/null || true)"
      if [[ -z "$repo_path" || "$repo_path" == "." || "$repo_path" == "<plugin-root>" ]]; then
        repo="${WORKSPACE:t}"
      else
        repo="${repo_path:t}"
      fi
