...
def main() -> int:
    ...
    runnable = [FakeActivity("execution-system-testing", {"autopilot": "true", "status": "ready"})]
    ...
    if code == 0:
        raise AssertionError("disallowed focus outside the execution-system line should fail")
    expect(output, "EXECUTION_SYSTEM_MAINTENANCE_FAILED")
    expect(output, "- reason: current focus is neither runnable work outside the execution-system line nor an approved non-runnable focus")
