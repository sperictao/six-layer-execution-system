...
def main() -> int:
    with cloned_workspace() as workspace:
        ...
        original = heartbeat.read_text(encoding="utf-8")
        if TARGET_LINE not in original:
            raise AssertionError("target heartbeat line missing before test")
        ...
        if governance.returncode == 0:
            raise AssertionError("governance checker should fail when heartbeat recovery rule drifts")
        expect_contains(governance_output, "EXECUTION_SYSTEM_GOVERNANCE_CONSISTENCY_FAILED")
        expect_contains(
            governance_output,
            "HEARTBEAT: missing `heartbeat 与人工触发必须遵守同一套恢复规则`",
        )
        ...
        expect_contains(
            runner_output,
            "- recovery_hint: inspect spec/AGENTS/HEARTBEAT alignment and repair the drifted recovery rule",
        )
