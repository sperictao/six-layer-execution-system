    ...
    with cloned_workspace() as workspace:
        ...
        if extract_activity_type(original, focus_id) != "roadmap":
            for activity_id in (
                ...
                "active-ledger-v2",
                "one-publish-refactor",
            ):
                if f"### Activity: {activity_id}\n" in original:
                    focus_id = activity_id
                    break

        test_source = original.replace(str(WORKSPACE), str(workspace))
        ...
        test_source = replace_focus_scalar(test_source, focus_id, "path", str(workspace))
        test_source = replace_focus_scalar(test_source, focus_id, "last_commit", baseline_commit)
